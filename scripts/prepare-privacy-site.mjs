...
function assertSafeOutputPath(path) {
  ...
  if (
    ...
    throw new Error('FlowSnap: privacy site output must use test-results/privacy-site.');
  }
}
...
export function validatePrivacyPolicyHtml(html, configuration) {
  ...
  try {
    ...
    throw new Error('FlowSnap: the public privacy policy URL is invalid.');
  }

  if (parsedUrl.protocol !== 'https:') {
    throw new Error('FlowSnap: the public privacy policy URL must use HTTPS.');
  }
  ...
  for (const content of requiredContent) {
    if (!html.includes(content)) {
      throw new Error(
        `FlowSnap: privacy policy is missing required content: ${content}`,
      );
    }
  }

  if (/<script\b/i.test(html)) {
    throw new Error('FlowSnap: the privacy policy must not contain scripts.');
  }
}
...
export function validatePrivacyDocumentation(markdown, configuration) {
  if (!markdown.includes(configuration.publicUrl)) {
    throw new Error(
      'FlowSnap: Privacy practices documentation does not use the public policy URL.',
    );
  }
  if (!markdown.includes('`public/privacy.html` is the only editable policy source.')) {
    throw new Error(
      'FlowSnap: Privacy practices documentation does not identify the canonical source.',
    );
  }
}
...
export async function preparePrivacySite() {
  ...
  if (JSON.stringify(files) !== JSON.stringify(PRIVACY_SITE_FILES)) {
    throw new Error(
      `FlowSnap: privacy site contains unexpected files: ${files.join(', ')}`,
    );
  }
...
if (isDirectExecution) {
  ...
  console.log(
    `Prepared the FlowSnap privacy site with ${result.files.length} files at ${result.outputRoot}.`,
  );
}
